...
std::list<std::string> PlayerbotHolder::HandlePlayerbotCommand(char const* args, Player* master)
{
	...
	if (!strcmp(cmd, "self"))
	{
		...
		if (!buddy)
		{
			...
			{
				/*if (!ai->HasRealPlayerMaster())
					OnBotLoginInternal(master);*/

				playerBots[master->GetGUIDLow()] = master;

				buddy->SetMaster(master);

				/*if (master->GetGroup() && !master->GetGroup()->IsLeader(masterGuid) && !sPlayerbotAIConfig.IsFreeAltBot(master))
					master->GetGroup()->ChangeLeader(masterGuid);*/
				...
				//	{
				//		WorldPacket p;
				//		std::string member = master->GetName();
				//		p << uint32(PARTY_OP_LEAVE) << member << uint32(0);
				//		master->GetSession()->HandleGroupDisbandOpcode(p);
				//	}
				...
				buddy->TellPlayer(buddy->GetMaster(), BOT_TEXT("Self bot initialized."));
