        ...
        for (uint32 i = 0; i < sChatChannelsStore.GetNumRows(); ++i)
        {
            ...
            switch (channel->ChannelID)
            {
                ...
                case ChatChannelId::GUILD_RECRUITMENT: {
                    ...
                    snprintf(new_channel_name_buf, 100, channel->pattern[locale], bot->GetPlayerbotAI()->GetLocalizedAreaName(GetAreaEntryByAreaID(ImportantAreaId::CITY)).c_str());
...
std::list<std::string> PlayerbotHolder::HandlePlayerbotCommand(char const* args, Player* master)
{
    ...
    if (!strcmp(cmd, "always"))
    {
        ...
        if (always == BotAlwaysOnline::DISABLED || always == BotAlwaysOnline::DISABLED_BY_COMMAND)
        {
            ...
            else
            {
                ...
                if (bot)
                {
                    ProcessBotCommand("add", guid, master->GetObjectGuid(), master->GetSession()->GetSecurity() >= SEC_GAMEMASTER, master->GetSession()->GetAccountId(),
                                      master->GetGuildId());
                }
                else
                {
        ...
        else
        {
            ...
            auto it = remove_if(sPlayerbotAIConfig.freeAltBots.begin(), sPlayerbotAIConfig.freeAltBots.end(),
                                [guid](std::pair<uint32, uint32> i) { return i.second == guid.GetCounter(); });
    ...
    if (!strcmp(cmd, "self"))
    {
        ...
        //        buddy = master->GetPlayerbotAI();

        //        if (buddy)
        //        {
        //            playerBots[master->GetGUIDLow()] = master;

        //            buddy->SetMaster(master);

        //            buddy->ResetStrategies();

        //            if (master && !master->IsTaxiFlying())
        //            {
        //                master->GetMotionMaster()->MovementExpired();
        //            }

        //            // check activity
        //            buddy->AllowActivity(ALL_ACTIVITY, true);
        //            // set delay on login
        //            buddy->SetActionDuration(urand(2000, 4000));

        //            buddy->TellPlayer(buddy->GetMaster(), "Self bot initialized.");

        //            //JoinChatChannels(master);

        //            /*if (sRandomPlayerbotMgr.IsRandomBot(master))
        //{
        //	uint32 lowguid = master->GetObjectGuid().GetCounter();
        //	auto result = CharacterDatabase.PQuery("SELECT 1 FROM character_social WHERE flags='%u' and friend='%d'", SOCIAL_FLAG_FRIEND, lowguid);
        //	if (result)
        //		master->GetPlayerbotAI()->SetPlayerFriend(true);
        //	else
        //		master->GetPlayerbotAI()->SetPlayerFriend(false);

        //	if (sPlayerbotAIConfig.instantRandomize && !sPlayerbotAIConfig.disableRandomLevels && !master->GetTotalPlayedTime())
        //	{
        //		sRandomPlayerbotMgr.InstaRandomize(master);
        //	}
        //}

        //if (!master->HasItemCount(6948, 1))
        //{
        //	master->StoreNewItemInBestSlots(6948, 1);
        //}*/
        //            messages.push_back("Enabled player ai");
        //        }
        //    }
        //    else
        //    {
        //        //DisablePlayerBot(master->GetGUIDLow());
        //        messages.push_back("Already self botting.");
        //    }
    ...
    for (std::vector<std::string>::iterator i = chars.begin(); i != chars.end(); i++)
    {
        ...
        auto results = CharacterDatabase.PQuery("SELECT name FROM characters WHERE account = '%u'", accountId);
        if (results)
        {
            do
            {
    ...
    for (std::set<std::string>::iterator i = bots.begin(); i != bots.end(); ++i)
    {
        ...
        {
            out << ProcessBotCommand(cmdStr, member, master->GetObjectGuid(), master->GetSession()->GetSecurity() >= SEC_GAMEMASTER, master->GetSession()->GetAccountId(),
                                     master->GetGuildId());
        }
        else if (!master)
        {
...
std::string PlayerbotHolder::ListBots(Player* master)
{
    ...
    if (master)
    {
        auto results = CharacterDatabase.PQuery("SELECT class,name FROM characters where account = '%u'", master->GetSession()->GetAccountId());
        if (results != NULL)
        {
            do
            {
...
PlayerbotMgr::PlayerbotMgr(Player* const master) : PlayerbotHolder(), master(master), lastErrorTell(0)
{
    auto newAI = std::make_unique<BuddybotAI>(master);
    master->CreatePlayerbotAI(std::move(newAI));
    auto buddy = dynamic_cast<BuddybotAI*>(master->GetPlayerbotAI());
    ...
    if (buddy)
    {
        ...
        buddy->SetMaster(master);
        buddy->SetCheat(BotCheatMask::none);

        buddy->ResetStrategies();

        buddy->ResetBuddyStrategies();
...
{
    ForEachPlayerbot([&](Player* bot) { bot->GetPlayerbotAI()->HandleMasterIncomingPacket(packet); });
...
{
    ForEachPlayerbot([&](Player* bot) { bot->SaveToDB(); });
...
void PlayerbotMgr::OnPlayerLogin(Player* player)
{
    ...
    sLog.outBasic("Player %s logged in, localeDbc %i, localeDb %i", player->GetName(), (uint32)(player->GetSession()->GetSessionDbcLocale()),
    ...
    auto results     = CharacterDatabase.PQuery("SELECT guid, name FROM characters WHERE account = '%u'", accountId);
    if (results)
    {
        std::ostringstream out;
        out << "add ";
        ...
        do
        {
            ...
            if (first)
                ...
                out << ",";
            if (sPlayerbotAIConfig.botAutologin == BotAutoLogin::LOGIN_ONLY_ALWAYS_ACTIVE && !sPlayerbotAIConfig.IsFreeAltBot(fields[0].GetUInt32()))
                continue;
